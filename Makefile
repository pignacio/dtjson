...
lint:
	...
	-pep8 dtjson tests
	-find dtjson -name "*.py" -exec pylint {} +
...
test-cover: test-deps clean-pyc
	python setup.py nosetests --with-coverage --cover-package=dtjson
...
coverage: test-deps clean-pyc
	coverage run --source dtjson setup.py nosetests
	make coverage-show
...
docs: docs-deps
	rm -f docs/dtjson.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ dtjson
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
yapf:
	find dtjson -name "*.py" -exec echo {} \; -exec yapf --diff {} \;
